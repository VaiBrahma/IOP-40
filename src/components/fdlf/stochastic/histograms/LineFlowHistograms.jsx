...
const LineFlowHistograms = () => {
  const stochasticFDLFresult = useSelector(state => state.stochasticResults);
  const lineFlow_samples = stochasticFDLFresult?.Pline_samples || [];

  const histogramData = useMemo(() => {
    return lineFlow_samples.map(samples => createHistogramData(samples));
  }, [lineFlow_samples]);

  if (lineFlow_samples.length === 0) {
    return <div className="p-4 text-center">No line flow samples available.</div>;
  }
...
export default LineFlowHistograms;
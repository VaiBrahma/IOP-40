...
const createHistogramData = (samples, bins = 30) => {
  ...
  return binCounts.map((count, i) => {
    ...
    return {
      bin: x.toFixed(3),
      count,
      pdf: pdf * valid.length * binWidth,
    };
  });
};

const AngleHistograms = () => {
  const stochasticFDLFresult = useSelector(state => state.stochasticResults);
  const delta_samples = stochasticFDLFresult?.delta_samples || [];

  const histogramData = useMemo(() => {
    return delta_samples.map(samples => createHistogramData(samples));
  }, [delta_samples]);

  if (delta_samples.length === 0) {
    return <div className="p-4 text-center">No angle samples available.</div>;
  }
...
export default AngleHistograms;
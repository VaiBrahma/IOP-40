...
const LineFlow = () => {
  ...
  useEffect(() => {
    const samples = stochasticFDLFresult?.Pline_samples;
    if (samples?.length > 0) {
      const boxPlotData = samples.map((lineSamples, i) => ({
        x: `Line ${i + 1}`,
        y: calculateBoxValues(lineSamples),
      }));

      setSeries([{ name: "Line Flow", type: "boxPlot", data: boxPlotData }]);
  ...
  return (
    <div>
      <BoxPlot series={series} yMin={yLimits.min} yMax={yLimits.max} text="Line Flow Distribution (Stochastic FDLF)"/>
    </div>
  );
};
